...
page_RB_setup = div( # CHANGE THIS
  titlePanel("Section 3.1: Relative Belief Setup"), # CHANGE THIS
  sidebarLayout(
...
RB_generate_dataframe = function(prior, post, relative_belief){
  # This loop is to bring back the NA values to ensure users
  # do not misuse when downloaded.
  for(i in 1:length(prior)){
    if(is.na(post[i]/prior[i])){
      relative_belief[i] = NA
    }
  }
  RB_df = data.frame(prior, post, relative_belief)
  colnames(RB_df) = c("Prior", "Posterior", "Relative Belief")
  return(RB_df)
}

#RB_dataframe_startrow = function(prior, post, relative_belief){
#  # since there's a lot of 0 and NaNs, prints starting from a different row
#  
#}
...
#y = RB_compute_values(alpha1w, alpha2w, n, nD)
#test = RB_generate_dataframe(y$prior, y$post, y$relative_belief)
